...
def ResNet_50(num_classes, num_vc=0, dropout_rate=0, layer_to_vcl=5, p_h=1, p_w=1, total_sub_models_num=5):
    m = ResNet50(classes=num_classes, input_shape=(224, 224, 3), weights='imagenet', include_top=False)
    layer_to_vcl_dict = {1: 'activation_6', 2: 'activation_18', 3: 'activation_33', 4: 'activation_39', 5: 'activation_48'}
    ...
    if num_vc > 0:
        ...
        if layer_to_vcl == total_sub_models_num:
            model = models.Model(inputs=m.input, outputs=output, name='ResNet50')
        else:
            layer_to_next_vcl = m.get_layer(layer_to_vcl_dict[layer_to_vcl + 1]).output
            ...
            model = models.Model(inputs=m.input, outputs=[output, output2], name='ResNet50')
        return model, vcl, m.input, vcl_next

    else:
        x = layers.GlobalAveragePooling2D(name='avg_pool')(m.layers[-1].output)
        x = layers.Dropout(rate=dropout_rate)(x)
        output = layers.Dense(num_classes, activation='softmax', name='fc1000', kernel_regularizer=l2(0.001))(x)
        model = models.Model(inputs=m.input, outputs=output, name='ResNet50')
        return model
